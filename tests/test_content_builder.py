...
class ContentBuilderTestCase(unittest.TestCase):
    def test_build_final_content_includes_blocks(self):
        note = NoteDetails(id="n1", title="Draft", body="Raw body")
        ...
        final_content = build_final_content(note, result, related, similarity_threshold=0.8)
        ...
        self.assertIn("Оригінальна чернетка (Backup)", final_content)
